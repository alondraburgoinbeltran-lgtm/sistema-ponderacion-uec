...
function uploadModal(){
    ...
    <div class="modalbox catalog-upload-modal">
      <div class="modal-head">
        <div>
          ...
          <p class="subtitle">Carga el PDF, revisa los entes identificados y confirma únicamente los que deban formar parte del catálogo.</p>
        </div>
      ...
      <div class="pdf-process-box">
        <div>
          <b>Extracción automática de entes</b>
          <p>El archivo se procesa en tu navegador. Antes de guardar, podrás corregir, eliminar o agregar nombres.</p>
        </div>
      ...
      <section id="previewSection" class="catalog-preview" hidden>
        <div class="catalog-preview-head">
          <div>
            ...
            <div class="subtitle"><span id="candidateCount">0</span> entes identificados · revisa la lista antes de guardar.</div>
          </div>
      ...
      <div class="modal-actions">
        ...
        <button class="btn primary" id="saveCat" disabled>Guardar catálogo</button>
      </div>
    </div>`;
  ...
  function appendCandidate(name=''){
    const tr = document.createElement('tr');
    tr.innerHTML = `
      <td class="catalog-check-col">
        <input class="candidate-include" type="checkbox" checked aria-label="Incluir ente">
      </td>
      <td>
        <input class="candidate-name" type="text" value="${escapeHtmlAttr(name)}" placeholder="Nombre del ente fiscalizado">
      </td>
  ...
  function renderCandidates(names){
    tbody.innerHTML = '';
    names.forEach(name => appendCandidate(name));
  ...
  $('#extract').onclick = async () => {
    ...
    setMessage('loading',`Procesando <b>${escapeHtml(file.name)}</b>… Esto puede tardar unos segundos.`);

    try{
      const lines = await extractPdfLines(file);
      const candidates = identifyAuditEntities(lines);

      if(!candidates.length){
        renderCandidates([]);
        setMessage(
          'warning',
          'El PDF se pudo leer, pero no fue posible identificar entes con suficiente seguridad. Puedes usar <b>＋ Agregar ente</b> para capturarlos sin volver a cargar el archivo.'
        );
      }else{
        renderCandidates(candidates);
        setMessage(
          'success',
          `Se identificaron <b>${candidates.length} posibles entes</b>. Revisa los nombres, elimina cualquier renglón que no corresponda y después guarda el catálogo.`
        );
      }
      ...
      setMessage(
        'error',
        'No fue posible leer automáticamente este PDF. Puede tratarse de un documento escaneado como imagen. Puedes agregar los entes manualmente con <b>＋ Agregar ente</b> o usar un PDF con texto seleccionable.'
      );
  ...
  saveBtn.onclick = async () => {
    ...
    const names = rows
      ...
      .map(row => row.querySelector('.candidate-name')?.value.trim())
      .filter(Boolean);

    const unique = [...new Map(names.map(name => [normalizeEntityKey(name), name])).values()];
    ...
    catalogs[year] = unique.map(name => ({name, type:'', work:null}));

    const file = $('#pdfFile').files[0];
    const programs = clone(store.get('programs',[]));

    if(file){
      /* Evita registrar el mismo programa dos veces si se vuelve a guardar. */
      const alreadyExists = programs.some(p => p.year===year && p.name===file.name);
      if(!alreadyExists){
        programs.push({year, name:file.name, date:new Date().toISOString()});
      }
    }

    saveBtn.disabled = true;
    saveBtn.textContent = 'Guardando…';
    setMessage('loading','Guardando el catálogo en Cloudflare D1…');
...
/* =========================================================
   8.1 LECTURA DEL PDF
   PDF.js conserva la posición de cada fragmento. Agrupamos
   los fragmentos por renglón para reconstruir mejor tablas.
   ========================================================= */
async function extractPdfLines(file){
  ...
  for(let pageNumber=1; pageNumber<=pdf.numPages; pageNumber++){
    ...
    for(const item of content.items){
      ...
      /* Une elementos cuya coordenada vertical sea prácticamente la misma. */
...
/* =========================================================
   8.2 IDENTIFICACIÓN DE ENTES
   Busca denominaciones institucionales típicas de un PAA y
   descarta encabezados, texto normativo y títulos genéricos.
   La lista SIEMPRE se muestra para revisión humana.
   ========================================================= */
function identifyAuditEntities(lines){
  const institutionWords = /\b(ayuntamiento|municipio|congreso|poder ejecutivo|poder judicial|tribunal|instituto|secretar[ií]a|organismo|sistema\s+(?:estatal|municipal)?\s*dif|comisi[oó]n|universidad|fideicomiso|colegio|consejo|junta|centro|procuradur[ií]a|contralor[ií]a|auditor[ií]a superior|instituci[oó]n|hospital|servicios de salud|instituto sudcaliforniano|patronato)\b/i;

  const rejectWords = /\b(programa anual|programa de auditor[ií]as|auditor[ií]a de desempe[nñ]o|auditor[ií]a financiera|auditor[ií]a de cumplimiento|cuenta p[uú]blica|ejercicio fiscal|tipo de auditor[ií]a|n[uú]mero de auditor[ií]a|objetivo|alcance|fundamento|marco legal|p[aá]gina|total de auditor[ií]as|calendario|cronograma|fecha de inicio|fecha de t[eé]rmino)\b/i;
  ...
  for(let raw of lines){
    ...
    if(line.length < 5 || line.length > 155) continue;
  ...
  /* Algunas tablas separan el número de fila y el ente; eliminamos duplicados. */
  const deduped = [];
  const seen = new Set();
